...
export default function NotificationBell() {
  ...
  return (
    <div>
      ...
      <div className="absolute mt-2 bg-white border shadow p-2 max-w-sm">
        {notifications.length === 0 && <p>No notifications</p>}
        {notifications.map((n) => (
          <div key={n.id} className={`p-2 ${n.isRead ? 'text-gray-500' : 'text-black'}`}>
            <p>
              Someone replied: <strong>{n.comment.content}</strong>
            </p>
            {!n.isRead && (
              <button className="text-blue-500 text-sm" onClick={() => markAsRead(n.id)}>
                Mark as Read
              </button>
            )}
          </div>
